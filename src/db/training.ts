...
export class PostgresTrainingSessionStore implements TrainingSessionStore {
  ...
  async create(input: { trainerAccountId: string; conversationId: string; targetRepository: string; originalQuestion: string }, queue = false) { return this.database.transaction(async transaction => { const now = new Date(); const [row] = await transaction.insert(trainingSessions).values({ id: randomUUID(), ...input, state: queue ? "GENERATING" : "ACTIVE", createdAt: now, updatedAt: now, version: 1 }).returning(); if (queue) await transaction.insert(jobs).values({ id: randomUUID(), kind: "GENERATE_TRAINING_ANSWER", payload: { sessionId: row!.id, version: row!.version }, status: "READY", availableAt: now, createdAt: now, updatedAt: now }); return fromRow(row!); }); }
  ...
  private async update(id: string, trainerAccountId: string, expected: TrainingState[], patch: Partial<typeof trainingSessions.$inferInsert>, kind: string, content: Readonly<Record<string, unknown>>, expectedVersion?: number, answerId?: string, queue = false) { return this.database.transaction(async (transaction) => { const [current] = await transaction.select().from(trainingSessions).where(and(eq(trainingSessions.id, id), eq(trainingSessions.trainerAccountId, trainerAccountId))).for("update"); if (!current) throw new TrainingStateError("TRAINING_NOT_FOUND", "Training session was not found."); if (!expected.includes(current.state as TrainingState) || (expectedVersion !== undefined && (current.version !== expectedVersion || (answerId !== undefined && current.currentAnswer?.id !== answerId)))) throw new TrainingStateError("INVALID_TRAINING_STATE", "That action is not available in the current training state or the answer changed."); const [{ count }] = await transaction.select({ count: sql<number>`count(*)::int` }).from(trainingTurns).where(eq(trainingTurns.sessionId, id)); await transaction.insert(trainingTurns).values({ id: randomUUID(), sessionId: id, ordinal: count + 1, actor: kind === "ANSWER" || kind === "REPORT" ? "AGENT" : "TRAINER", kind, content, createdAt: new Date() }); const now = new Date(); const [row] = await transaction.update(trainingSessions).set({ ...patch, updatedAt: now, version: current.version + 1 }).where(eq(trainingSessions.id, id)).returning(); if (queue) await transaction.insert(jobs).values({ id: randomUUID(), kind: "GENERATE_TRAINING_ANSWER", payload: { sessionId: id, version: row!.version }, status: "READY", availableAt: now, createdAt: now, updatedAt: now }); return fromRow(row!); }); }
  async saveAnswer(id: string, trainerAccountId: string, answer: Readonly<Record<string, unknown>>, insight?: string, expectedVersion?: number) { return this.update(id, trainerAccountId, ["ACTIVE", "GENERATING", "REVISING", "REPORT_READY"], { currentAnswer: answer, currentReport: null, state: "ACTIVE", answerError: null }, "ANSWER", { answer, insight: insight ?? null }, expectedVersion); }
  async saveFeedback(id: string, trainerAccountId: string, expectedVersion: number, answerId: string, feedback: string, queue = false) { return this.update(id, trainerAccountId, ["ACTIVE"], { state: "REVISING", answerError: null }, "FEEDBACK", { answerId, feedback }, expectedVersion, answerId, queue); }
  async saveClarification(id: string, trainerAccountId: string, expectedVersion: number, answerId: string, response: string, queue = false) {
    ...
    return this.update(id, trainerAccountId, ["ACTIVE"], { state: "REVISING", answerError: null }, "CLARIFICATION", { answerId, question: current.currentAnswer!.clarifyingQuestion, response }, expectedVersion, answerId, queue);
  }
  async retryAnswer(id: string, trainerAccountId: string, expectedVersion: number) { return this.database.transaction(async transaction => { const [current] = await transaction.select().from(trainingSessions).where(and(eq(trainingSessions.id, id), eq(trainingSessions.trainerAccountId, trainerAccountId))).for("update"); if (!current || current.version !== expectedVersion || !current.answerError || (current.state !== "REVISING" && !(current.state === "ACTIVE" && !current.currentAnswer))) throw new TrainingStateError("INVALID_TRAINING_STATE", "Reload the session before retrying the answer."); const now = new Date(); const [row] = await transaction.update(trainingSessions).set({ state: current.currentAnswer ? "REVISING" : "GENERATING", answerError: null, updatedAt: now, version: current.version + 1 }).where(eq(trainingSessions.id, id)).returning(); await transaction.insert(jobs).values({ id: randomUUID(), kind: "GENERATE_TRAINING_ANSWER", payload: { sessionId: id, version: row!.version }, status: "READY", availableAt: now, createdAt: now, updatedAt: now }); return fromRow(row!); }); }
  async failAnswer(id: string, version: number, message: string) { await this.database.update(trainingSessions).set({ state: sql`CASE WHEN ${trainingSessions.currentAnswer} IS NULL THEN 'ACTIVE' ELSE 'REVISING' END`, answerError: message, updatedAt: new Date(), version: sql`${trainingSessions.version}+1` }).where(and(eq(trainingSessions.id, id), eq(trainingSessions.version, version), sql`${trainingSessions.state} IN ('GENERATING','REVISING')`, sql`${trainingSessions.answerError} IS NULL`)); }
