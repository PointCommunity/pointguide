...
export class MemoryTrainingSessionStore implements TrainingSessionStore {
  ...
  async create(input: { trainerAccountId: string; conversationId: string; targetRepository: string; originalQuestion: string }, queue = false) { const now = new Date().toISOString(); const value: TrainingSessionRecord = { id: randomUUID(), ...input, state: queue ? "GENERATING" : "ACTIVE", currentAnswer: null, currentReport: null, proposalId: null, answerError: null, createdAt: now, updatedAt: now, version: 1 }; this.values.set(value.id, value); this.turns.set(value.id, []); return copy(value); }
