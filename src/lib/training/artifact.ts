...
import type { TrainingSourceContent } from "./types";
import { acceptedTrainingSources } from "./sources";
...
export function acceptedTrainingArtifact(session: TrainingSessionRecord, actorId: string, sourceCommit: string, acceptedAt: string, sources: TrainingSourceContent[] = []) {
  ...
  const content = JSON.stringify({
    ...
    applicability: { question: session.originalQuestion }, answer, trainerSources: acceptedTrainingSources(session.id, sources),
