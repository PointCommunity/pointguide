...
export async function generateAnswer(profile: ExecutionProfile, question: string, evidence: EvidenceItem[], runtime: ModelRuntime): Promise<AnswerDraft> {
  return parseJson(await generate(profile, evidencePrompt(question, evidence, profile.ownerPrompt), runtime), answerDraftSchema);
}
