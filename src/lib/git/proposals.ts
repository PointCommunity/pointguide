...
export function allowedSourceRepository(repository: string): boolean { return /^PointCommunity\/[A-Za-z0-9._-]+$/u.test(repository); }
...
export class ChangeProposal {
  ...
  constructor(input:{id:string;targetRepository:string;targetPath:string;operation:"CREATE"|"UPDATE"|"SUPERSEDE";proposedContent:string}) {
    ...
    if (!allowedSourceRepository(input.targetRepository)) throw new Error("Target repository is not configured.");
