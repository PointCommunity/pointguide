...
async function main() {
  ...
  const errors = compareProjectSnapshot(blueprint, snapshot, {
    ...
    visibility: "PRIVATE", // PointGuide's explicit repository policy, independent of template visibility.
    repository: `${profile.repository.owner}/${profile.repository.name}`,
    workflows: {
      "Auto-close issue": false,
      "Item added to project": false,
      "Item closed": false,
      "Pull request linked to issue": false,
    }, // Agent-owned transitions are stronger than the upstream automatic defaults.
  });
