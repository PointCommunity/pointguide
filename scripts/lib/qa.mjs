...
export function validateQA(qa) {
  ...
  for (const turn of qa.turns) {
    if (turn.pmGate !== undefined && !['local', 'release'].includes(turn.pmGate)) throw new Error('turn.pmGate must be local or release');
...
export function evaluateQA(qa, candidate, records) {
  ...
  for (const [index, turn] of qa.turns.entries()) {
    ...
    if (turn.pmGate !== 'release' && (record.pm?.owner !== turn.pm || record.pm.phrase !== turn.approvalPhrase || !matches(qa.candidateIdentity, candidate, record.pm.candidate) || typeof record.pm.evidence !== 'string' || !record.pm.evidence.trim())) return result('waiting', 'Exact-candidate PM Testing approval required.');
  ...
  const releasePM = qa.turns.some(turn => turn.pmGate === 'release');
  return { state: 'complete', baton, current: null, next: null, projectStatus: releasePM ? 'In Progress' : 'In Review', reason: 'Local QA complete; release and Issue completion gates remain separate.' };
}
