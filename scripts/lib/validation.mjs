...
export function compareProjectSnapshot(blueprint, snapshot, target = blueprint.workingExample) {
  validateProjectBlueprint(blueprint);
  const errors = [];
  if (snapshot.title !== target.title) {
    errors.push(`title mismatch: expected ${target.title}; found ${snapshot.title}`);
  }
  const expectsPublic = target.visibility === "PUBLIC";
  if (snapshot.public !== expectsPublic) {
    errors.push(`visibility mismatch: expected ${target.visibility}`);
  }
  if (!snapshot.repositories.includes(target.repository)) {
    errors.push(`repository link missing: ${target.repository}`);
  }
  ...
  for (const expected of blueprint.workflows) {
    const enabled = target.workflows?.[expected.name] ?? expected.enabled;
    const actual = workflows.get(expected.name);
    if (!actual) errors.push(`workflow missing: ${expected.name}`);
    else if (actual.enabled !== enabled) {
      errors.push(`workflow state mismatch for ${expected.name}: expected ${enabled}`);
    }
  }
  return errors;
}
