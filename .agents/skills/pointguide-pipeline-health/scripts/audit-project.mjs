...
import { isFrameworkMaintenance } from "./classify-pr.mjs";

const OWNER = "PointCommunity";
const REPO = `${OWNER}/pointguide`;
...
const REQUIRED_LABELS = new Set([
  ...
  "area:evidence", "area:identity", "area:training", "area:ui", "area:workflow",
]);
const errors = [];
...
if (project.title !== "PointGuide" || project.public !== false || project.closed !== false) {
  errors.push("Project must be the open private PointCommunity/PointGuide Project");
}
...
for (const pr of openPrs) {
  ...
  const changedFiles = gh(["pr", "diff", String(pr.number), "--repo", REPO, "--name-only"]).split("\n").filter(Boolean);
  if (isFrameworkMaintenance(pr, changedFiles)) continue;
...
for (const item of activeItems.filter((candidate) => candidate.status === "In Review")) {
  ...
  for (const [workflowName, checkName] of [["Quality", "verify"]]) {
...
if (errors.length) {
  console.error(`PointGuide pipeline health failed (${errors.length}):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

const activeSummary = activeItems.length ? `#${activeItems[0].content.number} ${activeItems[0].status}` : "none";
console.log(`PointGuide pipeline health OK: ${openIssues.length} open Issue(s), active=${activeSummary}, ${openPrs.length} open PR(s).`);
