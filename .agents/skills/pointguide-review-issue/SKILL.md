---
name: pointguide-review-issue
description: "Perform complete agent QA for the active PointGuide Issue, remediate all findings, deploy the exact candidate to Canary, and prepare the PM review checklist."
---

# Review a PointGuide Issue

Read `AGENTS.md`, `.agents/pointguide-pipeline-policy.html`, the active Issue, PR diff and history, acceptance criteria, current Canary state, and relevant test guidance.
...
5. Ensure the PR is current, references the Issue without auto-close syntax, and the GitHub `verify` plus recorded local AMD64 validation checks succeed for the exact committed head.
6. Invoke `pointguide-release-canary`. Do not request PM review until the exact candidate and whole homelab are verified green.
...
Wait for the exact phrase `Approved to merge and deploy production` for the current candidate. That phrase authorizes `pointguide-close-issue`.
