...
const chartName = track === "canary" ? "pointguide-canary" : "pointguide";
...
const temporaryRoot = mkdtempSync(path.join(os.tmpdir(), "pointguide-candidate-"));
...
try {
  ...
  const expected = `10.0.20.11:32309/pointguide:${sourceSha.slice(0, 7)}@${imageDigest}`;
  const imageLines = rendered.split("\n").filter((line) => line.trim().replaceAll('"', "") === `image: ${expected}`);
  if (imageLines.length !== 3) throw new Error(`expected 3 rendered image references for ${expected}; found ${imageLines.length}`);
