...
- Accepted Training is trainer-authorized, citable primary knowledge after exact publication, full source-contract validation, and activation; it is not independent manufacturer or site verification. Search applicable accepted answers first, preserve their complete steps, qualifications, warnings, and original supporting references, and supplement only uncovered parts. Surface conflicts and unknown applicability without claiming a complete answer. Keep the exact accepted artifact repository revision visible in Sources. Exclude raw feedback, unfinished output, unpublished, unindexed, archived, and superseded artifacts.
...
- A Trainer, Admin, or Owner accepting the exact completed Training answer authorizes only that immutable artifact under `research/pointguide-training/<session-id>/<answer-id>.json` in the visible active configured repository, its byte-identical trainer-provided originals under that session's `originals/`, its extracted text and provenance under `sources/`, and the deterministic session/folder README.md and root inventory/checksum updates required for that exact bundle. An unanswered essential clarification or unfinished source extraction is not a completed answer. The worker must derive and verify the exact dynamic allow-listed diff, every companion byte and digest, head, artifact digest, full source contract, ordinary merge without administrator bypass, resulting revision, and atomic activation before Ready to use. Retries preserve accepted bytes and the previous active snapshot. This exception never authorizes arbitrary README or corpus edits, the PointGuide application PR merge, an unrelated proposal, or live technology changes.
