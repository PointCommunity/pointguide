...
- A Trainer, Admin, or Owner accepting the exact completed Training answer authorizes only that immutable artifact under `research/pointguide-training/<session-id>/<answer-id>.json` in the visible active configured repository, plus its deterministic new session README.md and root inventory/checksum updates. An unanswered essential clarification is not a completed answer. The worker must verify the exact four-path diff, companion contents, head, artifact digest, full source contract, ordinary merge without administrator bypass, resulting revision, and atomic activation before Ready to use. Retries preserve accepted bytes and the previous active snapshot. This exception never authorizes arbitrary README or corpus edits, the PointGuide application PR merge, an unrelated proposal, or live technology changes.
